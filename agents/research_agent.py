...
def run_research_agent_from_input(user_input: str, *, fast: bool | None = None) -> dict:
    ...
    # These four steps only depend on market_scope / user_input, not on each
    # other's output, so run them concurrently instead of one after another.
    print(
        "[Research Agent] Gathering company knowledge, market search, "
        "competitors, and channels in parallel...",
        flush=True,
    )

    with ThreadPoolExecutor(max_workers=4) as executor:
        f_company = executor.submit(gather_company_knowledge.invoke, {"query": user_input})
        f_search = executor.submit(web_search.invoke, {"query": web_query})
        f_competitors = executor.submit(
            analyze_competitors.invoke,
            {"user_query": user_input, "market_scope": market_scope},
        )
        f_channels = executor.submit(
            analyze_marketing_channels.invoke,
            {"product": product, "audience": audience or "enterprise buyers"},
        )

        company_knowledge = f_company.result()
        market_search = f_search.result()
