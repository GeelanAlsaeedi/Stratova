...
# Guardrails only score/classify an already-written output — this does not
# need the same heavy model used for research/strategy/content generation.
# A small, fast model is enough and cuts guardrails latency significantly.
# Override with GUARDRAILS_MODEL in .env if needed.
_GUARDRAILS_MODEL_NAME = os.getenv("GUARDRAILS_MODEL", "gpt-4o-mini")
_guardrails_llm = ChatOpenAI(model=_GUARDRAILS_MODEL_NAME, temperature=0)
...
def call_guardrails_llm(context: str, prompt: str) -> str:
    try:
        response = _guardrails_llm.invoke(
            [
                {"role": "system", "content": GUARDRAILS_SYSTEM},
                {"role": "user", "content": f"{context}\n\n{prompt}"},
            ]
        )
        return response.content
    except Exception:
        # Fall back to the original shared-model path if the lightweight
        # model call fails for any reason (e.g. model not available).
        try:
