...
"""
    ...
    # seo_keywords, social_posts, email_campaign, ad_copy, and blog_articles
    # only depend on content_context + content_calendar, not on each other,
    # so they can all be generated concurrently instead of one-by-one.
    print(
        f"[Content Agent] Generating SEO, social, email, ads, and {blog_count} "
        "blog article(s) in parallel...",
        flush=True,
    )
    ...
    with ThreadPoolExecutor(max_workers=5) as executor:
        f_seo = executor.submit(generate_seo_keywords.invoke, calendar_args)
        f_social = executor.submit(generate_social_posts.invoke, calendar_args)
        f_email = executor.submit(generate_email_campaign.invoke, calendar_args)
        f_ads = executor.submit(generate_ad_copy.invoke, calendar_args)
        f_blog = executor.submit(generate_content, content_context, blog_prompt)

        seo_keywords = sanitize_content_language(f_seo.result())
        social_posts = sanitize_content_language(f_social.result())
        email_campaign = sanitize_content_language(f_email.result())
        ad_copy = sanitize_content_language(f_ads.result())
        blog_articles = sanitize_content_language(f_blog.result())
