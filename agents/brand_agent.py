...
def run_brand_agent_from_content(
    ...
    print(f"[Brand Agent] Evaluating {len(items)} content item(s) in parallel...", flush=True)

    def _evaluate_item(item: dict) -> dict:
        print(f"[Brand Agent] Reviewing: {item.get('title')}", flush=True)
        return evaluate_content_alignment.invoke(
            {
                "content_item": item,
                "brand_context": brand_context,
            }
    ...
    # Items are independent of each other, so evaluate them concurrently
    # instead of waiting on one LLM call at a time.
    with ThreadPoolExecutor(max_workers=min(6, max(1, len(items)))) as executor:
        evaluations = list(executor.map(_evaluate_item, items))
