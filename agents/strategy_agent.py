...
BRAND_IDENTITY_PDF = os.path.join(
    ROOT_DIR,
    "knowledge_base",
    "documents",
    DEFAULT_BRAND_IDENTITY_PDF
)
...
"""
    ...
    # icp and positioning both only need strategy_context, so run them
    # together. personas depends on icp, so it must wait for that step.
    print("[Strategy Agent] Building ICP and positioning in parallel...", flush=True)
    with ThreadPoolExecutor(max_workers=2) as executor:
        f_icp = executor.submit(build_icp.invoke, {"research_context": strategy_context})
        f_positioning = executor.submit(
            create_positioning_statement.invoke, {"research_context": strategy_context}
        )
        icp = f_icp.result()
        positioning = f_positioning.result()
    ...
    # messaging and channels both only need personas, so run them together.
    print("[Strategy Agent] Generating messaging pillars and channels in parallel...", flush=True)
    with ThreadPoolExecutor(max_workers=2) as executor:
        f_messaging = executor.submit(
            generate_messaging_pillars.invoke,
            {"research_context": strategy_context, "personas": personas},
        )
        f_channels = executor.submit(
            recommend_channels.invoke,
            {"research_context": strategy_context, "personas": personas},
        )
        messaging = f_messaging.result()
