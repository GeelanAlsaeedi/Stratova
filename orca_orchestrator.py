...
def merge_dicts(current: dict | None, update: dict | None) -> dict:
    """
    Reducer for state keys that can be updated concurrently by parallel
    branches (e.g. guardrails_results written by both the content/brand
    branch and the analysis branch running at the same time).
    """
...
class OrcaState(TypedDict, total=False):
    user_input: str
    guardrails_results: Annotated[dict, merge_dicts]
...
OUTPUT_ROOT = Path(ROOT_DIR) / "outputs" / "runs"
...
# "analysis" does not depend on research/strategy/content/brand_alignment
# (it runs on a fixed campaign dataset), so it starts at the same time as
# "research" instead of waiting for the whole content pipeline to finish.
# "final_output" has two incoming edges, so LangGraph waits for both
# branches to complete before running it (fan-in).
orca_graph.add_edge(START, "research")
orca_graph.add_edge(START, "analysis")

orca_graph.add_edge("research", "strategy")
orca_graph.add_edge("strategy", "content")
orca_graph.add_edge("content", "brand_alignment")
orca_graph.add_edge("brand_alignment", "final_output")
...
def run_orca(
    ...
    # Always create run_id up front so parallel branches (research + analysis)
    # both see the same run_id from the very first tick, instead of relying
    # on research_node to create it.
    ...
    initial_state = {
        ...
        "run_dir": str(get_run_dir(resolved_run_id)),
        **memory_state,
    }
